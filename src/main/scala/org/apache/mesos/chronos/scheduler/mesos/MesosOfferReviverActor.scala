...
object MesosOfferReviverActor {
  ...
  def props(config: SchedulerConfiguration, mesosDriverFactory: MesosDriverFactory): Props = {
    Props(new MesosOfferReviverActor(config, mesosDriverFactory))
  }
}
...
class MesosOfferReviverActor(
  ...
  mesosDriverFactory: MesosDriverFactory) extends Actor with ActorLogging {
